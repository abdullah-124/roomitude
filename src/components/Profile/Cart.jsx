    ...
    <div className="mx-auto">
      {
        items.length ? <div className='text-sm'>
          <h3 className='sticky top-0 text-lg font-medium px-5 py-2 shadow bg-white'>Cart List (<span>{cartCount}</span>)</h3>
          <div className={`max-h-[${max_height}] px-5 flex flex-col overflow-y-scroll shadow`}>
            {
              items.map((item, idx) => (
